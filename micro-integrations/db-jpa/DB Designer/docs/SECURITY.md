...
| R10 | Package download reliability: two vendor issues could hang the browser download. (a) `getEntityColumns()` settles its Promise only from inside a `forEach`, so for a flow with a data-transformation mapper it never resolves when the entity file is not at the expected flat path (hangs indefinitely). (b) The download base64-encodes the package in memory; the ~108MB static core jar alone is ~15s + a ~150MB payload that exceeds the UI client timeout (config+entity-only = 82ms/32KB). | Low | Mitigated in the startup patch: (a) `getEntityColumns` is replaced with a version that reads the flat entity tree, searches recursively, and always resolves -- verified end-to-end at 200/633ms; (b) the static core jar is dropped from the browser download by default (it ships in the bundle connector/ folder), and backend memory is 4Gi with a `NODE_OPTIONS` heap cap. Re-include the jar on native amd64 via `servicesApp.webDownloadIncludeCoreJar=true`. Streaming the zip as `application/zip` remains a future image update. |
