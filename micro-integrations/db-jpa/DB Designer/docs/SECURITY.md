...
| R10 | Inefficient package download: the browser download base64-encodes the whole connector package into an in-memory JSON response. The heavy item is the ~108MB static core connector jar -- zipping it takes ~15s and yields a ~150MB payload that exceeds the UI client timeout under emulation (measured in-pod: config+entity-only = 82ms/32KB vs +core jar = ~15s/~150MB). | Low | Mitigated: the startup patch drops the static core jar from the browser download by default (it ships in the bundle connector/ folder), so Config + entity.jar return in well under a second; backend memory is also raised to 4Gi with a `NODE_OPTIONS` heap cap. Re-include the jar on native amd64 via `servicesApp.webDownloadIncludeCoreJar=true`. Streaming the zip as `application/zip` instead of base64-in-JSON remains a future image update. |
