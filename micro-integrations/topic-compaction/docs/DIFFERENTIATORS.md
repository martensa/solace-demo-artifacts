...
| **Tombstones** | `null` payload at a key, fragile (some serializers reject null), interpreted by background cleaner | Explicit `DELETE` command (V2) or `DELETE /api/v1/kv/{key}` REST. Clean semantics, auditable |
...
| **Replay-on-demand** | Not supported. Consumers must rewind offsets and re-process | **Command-driven replay**: publish a JSON command, the MI republishes the latest value to `<key>/compacted` |
...
- **Multi-broker write throughput on a single topic** - Kafka's partitions
  scale write throughput linearly. This MI is single-instance per VPN today.
  A future V2 with active-active sharding would close this gap; in practice
  most compaction use cases are read-dominant.
- **Mature operator tooling** - Kafka has decades of dashboards, alerting,
  rebalancing tools. We rely on Spring Boot Actuator + Solace's standard
  monitoring.
